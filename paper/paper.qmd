...
ggplot(deaths_trend, aes(x = occ_year, y = Total_Deaths)) +
  ...
  theme(plot.title = element_text(size = 12)) 
```
**Figure 2.** *The line plot illustrates the trends of deaths due to shootings in Toronto from 2004 to 2024.*

```{r}
injuries_trend <- data %>%
  filter(!is.na(occ_year)) %>%  # Filter out missing year data
  group_by(occ_year) %>%  # Group by year
  summarise(Total_Injuries = sum(injuries, na.rm = TRUE))  # Sum of deaths per year

# Line plot for deaths trend over the years
ggplot(injuries_trend, aes(x = occ_year, y = Total_Injuries)) +
  geom_line(color = "darkred") +  # Line for the trend
  geom_point(color = "darkred") +  # Points for each year
  labs(title = "Trend of Deaths Due to Shootings Over the Years",
       x = "Year",
       y = "Total Number of Deaths") +
  theme_minimal() +
  scale_x_continuous(breaks = seq(min(injuries_trend$occ_year), max(injuries_trend$occ_year), by = 2))+
  theme(plot.title = element_text(size = 12)) 
```
**Figure 3.** *The line plot illustrates the trends of injuries due to shootings in Toronto from 2004 to 2024.*
